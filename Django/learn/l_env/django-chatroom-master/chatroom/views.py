...
def SignUp(request):
    if request.method == "POST":
        ...
        if userFrom.is_valid():
            ...
            password = make_password(userFrom.cleaned_data['password'], None, 'pbkdf2_sha256')
            try: # 重复检测
                User.objects.create(username = username, password = password)
            except IntegrityError:
                return render_to_response('registration/sign_up.html', {'tips':'注册失败,用户名重复'})
            # 注册成功，跳转到登录
            return render_to_response('registration/success.html')
        else:
            return render_to_response('registration/sign_up.html', {'tips':'注册失败,请正确输入'})
    else:
